...
function renderStateFlowItem([label, value]: [string, string]): string {
  const layoutClass = label === "Reason" ? " state-flow-reason" : label === "Review Reason" ? " state-flow-review" : "";
  return `<div class="state-flow-item${layoutClass}"><span>${escapeHtml(label)}</span><strong>${escapeHtml(value)}</strong></div>`;
}
