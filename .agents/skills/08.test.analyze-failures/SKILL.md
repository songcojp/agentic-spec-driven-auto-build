---
name: 08.test.analyze-failures
description: "Run the Agentic Spec test analyze failures workflow. Use when the scheduler, operator, or another skill explicitly requests `08.test.analyze-failures` and needs traceable test plans, test cases, failure analysis, acceptance mappings, or verification results inside the test and verification boundary."
---
...
1. Confirm the invocation requested `08.test.analyze-failures`, identify the target artifact or object, and verify the current state allows this analyze failures step.
