---
name: 12.recovery.capture-checkpoint
description: "Run the Agentic Spec recovery capture checkpoint workflow. Use when the scheduler, operator, or another skill explicitly requests `12.recovery.capture-checkpoint` and needs traceable recovery classification, checkpoint, restore, resume, blocked-state, or validation outputs inside the recovery boundary."
---

# Recovery Capture Checkpoint

## Purpose

Run the exact Agentic Spec recovery `capture-checkpoint` step for `12.recovery.capture-checkpoint`. This skill turns referenced inputs into recovery classification, checkpoint, restore, resume, blocked-state, or validation outputs while staying inside the recovery boundary. It must preserve existing IDs, states, and evidence links unless the invocation explicitly authorizes a change.

## Codex Skill Usage

Use this project-local skill only when the user, scheduler, or another skill explicitly names `12.recovery.capture-checkpoint` or the current SpecDrive workflow step requires it. Keep context lean: read referenced files from disk, pass paths/IDs/section anchors instead of pasted documents, and return the project-local Skill output contract rather than free-form prose. Provider YAML files under `agents/` are UI/provider prompt metadata only; subagent roles and fallback rules belong in `SKILL.md`.

## When to Use

Use this skill when the operator, scheduler, or another skill requests `12.recovery.capture-checkpoint` or when the current Agentic Spec phase requires this exact capability. Do not use it as a catch-all replacement for adjacent skills in the same phase.
...
1. Confirm the invocation requested `12.recovery.capture-checkpoint`, identify the target artifact or object, and verify the current state allows this capture checkpoint step.
