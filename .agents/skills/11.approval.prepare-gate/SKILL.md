---
name: 11.approval.prepare-gate
description: "Run the Agentic Spec approval prepare gate workflow. Use when the scheduler, operator, or another skill explicitly requests `11.approval.prepare-gate` and needs traceable approval gate packages, decisions, rejection/replan results, human-review requests, or resume decisions inside the approval-gate boundary."
---

# Approval Prepare Gate

## Purpose

Run the exact Agentic Spec approval `prepare-gate` step for `11.approval.prepare-gate`. This skill turns referenced inputs into approval gate packages, decisions, rejection/replan results, human-review requests, or resume decisions while staying inside the approval-gate boundary. It must preserve existing IDs, states, and evidence links unless the invocation explicitly authorizes a change.

## Codex Skill Usage

Use this project-local skill only when the user, scheduler, or another skill explicitly names `11.approval.prepare-gate` or the current SpecDrive workflow step requires it. Keep context lean: read referenced files from disk, pass paths/IDs/section anchors instead of pasted documents, and return the project-local Skill output contract rather than free-form prose. Provider YAML files under `agents/` are UI/provider prompt metadata only; subagent roles and fallback rules belong in `SKILL.md`.

## When to Use

Use this skill when the operator, scheduler, or another skill requests `11.approval.prepare-gate` or when the current Agentic Spec phase requires this exact capability. Do not use it as a catch-all replacement for adjacent skills in the same phase.
...
1. Confirm the invocation requested `11.approval.prepare-gate`, identify the target artifact or object, and verify the current state allows this prepare gate step.
