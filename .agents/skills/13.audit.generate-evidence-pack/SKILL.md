---
name: 13.audit.generate-evidence-pack
description: "Run the Agentic Spec audit generate evidence pack workflow. Use when the scheduler, operator, or another skill explicitly requests `13.audit.generate-evidence-pack` and needs traceable evidence, logs, matrices, and audit traceability updates inside the audit and evidence boundary."
---

# Audit Generate Evidence Pack

## Purpose

Run the exact Agentic Spec audit `generate-evidence-pack` step for `13.audit.generate-evidence-pack`. This skill turns referenced inputs into evidence, logs, matrices, and audit traceability updates while staying inside the audit and evidence boundary. It must preserve existing IDs, states, and evidence links unless the invocation explicitly authorizes a change.

## Codex Skill Usage

Use this project-local skill only when the user, scheduler, or another skill explicitly names `13.audit.generate-evidence-pack` or the current SpecDrive workflow step requires it. Keep context lean: read referenced files from disk, pass paths/IDs/section anchors instead of pasted documents, and return the project-local Skill output contract rather than free-form prose. Provider YAML files under `agents/` are UI/provider prompt metadata only; subagent roles and fallback rules belong in `SKILL.md`.

## When to Use

Use this skill when the operator, scheduler, or another skill requests `13.audit.generate-evidence-pack` or when the current Agentic Spec phase requires this exact capability. Do not use it as a catch-all replacement for adjacent skills in the same phase.
...
1. Confirm the invocation requested `13.audit.generate-evidence-pack`, identify the target artifact or object, and verify the current state allows this generate evidence pack step.
...
6. If this invocation generates or updates an evidence pack or other Spec
   document, run the mandatory quality review and repair loop from
