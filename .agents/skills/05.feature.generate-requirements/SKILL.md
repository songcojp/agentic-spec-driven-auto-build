...
Use this skill to perform the Agentic Spec `05` `feature` workflow step for `generate-requirements`. Keep the workflow reusable across Agentic Spec projects and avoid product-specific assumptions.

## When to Use

Use this skill when the operator, scheduler, or another skill requests `05.feature.generate-requirements` or when the current Agentic Spec phase requires this exact capability. Do not use it as a catch-all replacement for adjacent skills in the same phase.
