---
name: 01.prd.extract-goals
description: "Run the Agentic Spec PRD extract goals workflow. Use when the scheduler, operator, or another skill explicitly requests `01.prd.extract-goals` and needs traceable product-intent outputs that downstream requirements can consume inside the product-intent boundary."
---
...
1. Confirm the invocation requested `01.prd.extract-goals`, identify the target artifact or object, and verify the current state allows this extract goals step.
