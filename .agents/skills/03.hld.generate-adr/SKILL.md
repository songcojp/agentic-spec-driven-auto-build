...
description: "Execute the Agentic Spec 03 hld workflow for generate adr with reusable input references, output contract, and acceptance checks."
---
...
Use this skill to perform the Agentic Spec `03` `hld` workflow step for `generate-adr`. Keep the workflow reusable across Agentic Spec projects and avoid product-specific assumptions.

## When to Use

Use this skill when the operator, scheduler, or another skill requests `03.hld.generate-adr` or when the current Agentic Spec phase requires this exact capability. Do not use it as a catch-all replacement for adjacent skills in the same phase.
...
6. If this invocation generates or updates an ADR or other Spec document, run
