---
name: 02.requirements.update-traceability
description: "Run the Agentic Spec requirements update traceability workflow. Use when the scheduler, operator, or another skill explicitly requests `02.requirements.update-traceability` and needs traceable testable requirements, readiness findings, conflicts, or traceability updates inside the requirements and traceability boundary."
---

# Requirements Update Traceability

## Purpose

Run the exact Agentic Spec requirements `update-traceability` step for `02.requirements.update-traceability`. This skill turns referenced inputs into testable requirements, readiness findings, conflicts, or traceability updates while staying inside the requirements and traceability boundary. It must preserve existing IDs, states, and evidence links unless the invocation explicitly authorizes a change.

## Codex Skill Usage

Use this project-local skill only when the user, scheduler, or another skill explicitly names `02.requirements.update-traceability` or the current SpecDrive workflow step requires it. Keep context lean: read referenced files from disk, pass paths/IDs/section anchors instead of pasted documents, and return the project-local Skill output contract rather than free-form prose. Provider YAML files under `agents/` are UI/provider prompt metadata only; subagent roles and fallback rules belong in `SKILL.md`.

## When to Use

Use this skill when the operator, scheduler, or another skill requests `02.requirements.update-traceability` or when the current Agentic Spec phase requires this exact capability. Do not use it as a catch-all replacement for adjacent skills in the same phase.
...
1. Confirm the invocation requested `02.requirements.update-traceability`, identify the target artifact or object, and verify the current state allows this update traceability step.
...
3. Produce the testable requirements, readiness findings, conflicts, or traceability updates requested by the invocation, preserving stable IDs, states, source refs, and evidence links unless the invocation explicitly allows a change.
...
- The result stays within the `02` `requirements` boundary.
