---
name: 12.recovery.restore-checkpoint
description: "Run the Agentic Spec recovery restore checkpoint workflow. Use when the scheduler, operator, or another skill explicitly requests `12.recovery.restore-checkpoint` and needs traceable recovery classification, checkpoint, restore, resume, blocked-state, or validation outputs inside the recovery boundary."
---
...
1. Confirm the invocation requested `12.recovery.restore-checkpoint`, identify the target artifact or object, and verify the current state allows this restore checkpoint step.
