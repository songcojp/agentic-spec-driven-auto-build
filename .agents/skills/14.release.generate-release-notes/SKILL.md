---
name: 14.release.generate-release-notes
description: "Run the Agentic Spec release generate release notes workflow. Use when the scheduler, operator, or another skill explicitly requests `14.release.generate-release-notes` and needs traceable release-gate, PR-prep, release note, archive, or released-state outputs inside the release and delivery boundary."
---
...
Use this skill when the operator, scheduler, or another skill requests `14.release.generate-release-notes` or when the current Agentic Spec phase requires this exact capability. Do not use it as a catch-all replacement for adjacent skills in the same phase.
...
1. Confirm the invocation requested `14.release.generate-release-notes`, identify the target artifact or object, and verify the current state allows this generate release notes step.
...
6. If this invocation generates or updates release notes or other Spec
