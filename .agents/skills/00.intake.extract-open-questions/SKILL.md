---
name: 00.intake.extract-open-questions
description: "Run the Agentic Spec intake extract open questions workflow. Use when the scheduler, operator, or another skill explicitly requests `00.intake.extract-open-questions` and needs traceable normalized intake facts, constraints, or open questions inside the project-intake and source-normalization boundary."
---

# Intake Extract Open Questions

## Purpose

Run the exact Agentic Spec intake `extract-open-questions` step for `00.intake.extract-open-questions`. This skill turns referenced inputs into normalized intake facts, constraints, or open questions while staying inside the project-intake and source-normalization boundary. It must preserve existing IDs, states, and evidence links unless the invocation explicitly authorizes a change.

## Codex Skill Usage

Use this project-local skill only when the user, scheduler, or another skill explicitly names `00.intake.extract-open-questions` or the current SpecDrive workflow step requires it. Keep context lean: read referenced files from disk, pass paths/IDs/section anchors instead of pasted documents, and return the project-local Skill output contract rather than free-form prose. Provider YAML files under `agents/` are UI/provider prompt metadata only; subagent roles and fallback rules belong in `SKILL.md`.

## When to Use

Use this skill when the operator, scheduler, or another skill requests `00.intake.extract-open-questions` or when the current Agentic Spec phase requires this exact capability. Do not use it as a catch-all replacement for adjacent skills in the same phase.
...
1. Confirm the invocation requested `00.intake.extract-open-questions`, identify the target artifact or object, and verify the current state allows this extract open questions step.
...
3. Produce the normalized intake facts, constraints, or open questions requested by the invocation, preserving stable IDs, states, source refs, and evidence links unless the invocation explicitly allows a change.
...
- The result stays within the `00` `intake` boundary.
