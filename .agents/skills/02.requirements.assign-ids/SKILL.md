---
name: 02.requirements.assign-ids
description: "Execute the Agentic Spec 02 requirements workflow for assign ids with reusable input references, output contract, and acceptance checks."
---

# Requirements Assign Ids

## Purpose

Use this skill to perform the Agentic Spec `02` `requirements` workflow step for `assign-ids`. Keep the workflow reusable across Agentic Spec projects and avoid product-specific assumptions.

## When to Use

Use this skill when the operator, scheduler, or another skill requests `02.requirements.assign-ids` or when the current Agentic Spec phase requires this exact capability. Do not use it as a catch-all replacement for adjacent skills in the same phase.
