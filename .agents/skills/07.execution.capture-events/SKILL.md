---
name: 07.execution.capture-events
description: "Run the Agentic Spec execution capture events workflow. Use when the scheduler, operator, or another skill explicitly requests `07.execution.capture-events` and needs traceable execution references, events, run monitoring results, state updates, or implementation evidence inside the execution-state boundary."
---

# Execution Capture Events

## Purpose

Run the exact Agentic Spec execution `capture-events` step for `07.execution.capture-events`. This skill turns referenced inputs into execution references, events, run monitoring results, state updates, or implementation evidence while staying inside the execution-state boundary. It must preserve existing IDs, states, and evidence links unless the invocation explicitly authorizes a change.

## Codex Skill Usage

Use this project-local skill only when the user, scheduler, or another skill explicitly names `07.execution.capture-events` or the current SpecDrive workflow step requires it. Keep context lean: read referenced files from disk, pass paths/IDs/section anchors instead of pasted documents, and return the project-local Skill output contract rather than free-form prose. Provider YAML files under `agents/` are UI/provider prompt metadata only; subagent roles and fallback rules belong in `SKILL.md`.

## When to Use

Use this skill when the operator, scheduler, or another skill requests `07.execution.capture-events` or when the current Agentic Spec phase requires this exact capability. Do not use it as a catch-all replacement for adjacent skills in the same phase.
...
1. Confirm the invocation requested `07.execution.capture-events`, identify the target artifact or object, and verify the current state allows this capture events step.
