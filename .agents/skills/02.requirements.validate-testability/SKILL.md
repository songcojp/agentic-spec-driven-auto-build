...
This skill may run as a standalone requirements review or as the Quality Review
role inside a governed Spec document quality loop. Use Codex subagents only when
the operator, scheduler, or calling Skill has explicitly requested that
review/loop delegation. If real subagents are unavailable, run an isolated
owner-thread pass with the same reference-only input and compact-output rules.
...
7. When invoked from `02.requirements.convert-ears`, treat that call as the
   explicit review delegation for this pass. Run as an isolated review subagent
   when the runtime supports subagents; otherwise use an isolated owner-thread
   pass and record the fallback. Read the referenced PRD and requirements files
   directly from disk, classify each gap as repairable from the current source,
   requiring clarification, or requiring risk review, and return only the
   compact structured result. Do not return long-form analysis or copied
   requirement text unless a short excerpt is needed to identify the exact gap.
...
- Prefer a fresh subagent or isolated review context for every validation pass
  triggered by `02.requirements.convert-ears` or by
  `.agents/skills/SPEC_DOC_QUALITY_LOOP.md` after the caller explicitly enters
  that governed loop.
...
- If subagents are unavailable or not authorized in the current runtime, use the
  same compact-input and compact-output discipline in the owner context and
  record that fallback in `summary`.
