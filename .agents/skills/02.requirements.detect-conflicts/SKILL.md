---
name: 02.requirements.detect-conflicts
description: "Run the Agentic Spec requirements detect conflicts workflow. Use when the scheduler, operator, or another skill explicitly requests `02.requirements.detect-conflicts` and needs traceable testable requirements, readiness findings, conflicts, or traceability updates inside the requirements and traceability boundary."
---

# Requirements Detect Conflicts

## Purpose

Run the exact Agentic Spec requirements `detect-conflicts` step for `02.requirements.detect-conflicts`. This skill turns referenced inputs into testable requirements, readiness findings, conflicts, or traceability updates while staying inside the requirements and traceability boundary. It must preserve existing IDs, states, and evidence links unless the invocation explicitly authorizes a change.

## Codex Skill Usage

Use this project-local skill only when the user, scheduler, or another skill explicitly names `02.requirements.detect-conflicts` or the current SpecDrive workflow step requires it. Keep context lean: read referenced files from disk, pass paths/IDs/section anchors instead of pasted documents, and return the project-local Skill output contract rather than free-form prose. Provider YAML files under `agents/` are UI/provider prompt metadata only; subagent roles and fallback rules belong in `SKILL.md`.

## When to Use

Use this skill when the operator, scheduler, or another skill requests `02.requirements.detect-conflicts` or when the current Agentic Spec phase requires this exact capability. Do not use it as a catch-all replacement for adjacent skills in the same phase.
...
1. Confirm the invocation requested `02.requirements.detect-conflicts`, identify the target artifact or object, and verify the current state allows this detect conflicts step.
