---
name: 05.feature.update-status
description: "Run the Agentic Spec Feature Spec update status workflow. Use when the scheduler, operator, or another skill explicitly requests `05.feature.update-status` and needs traceable Feature Spec artifacts, scope findings, status updates, or feature index changes inside the Feature Spec package boundary."
---
...
1. Confirm the invocation requested `05.feature.update-status`, identify the target artifact or object, and verify the current state allows this update status step.
