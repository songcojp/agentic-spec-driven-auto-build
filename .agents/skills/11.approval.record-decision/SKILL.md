...
description: "Run the Agentic Spec approval record decision workflow. Use when the scheduler, operator, or another skill explicitly requests `11.approval.record-decision` and needs traceable approval gate packages, decisions, rejection/replan results, human-review requests, or resume decisions inside the approval-gate boundary."
---
...
1. Confirm the invocation requested `11.approval.record-decision`, identify the target artifact or object, and verify the current state allows this record decision step.
