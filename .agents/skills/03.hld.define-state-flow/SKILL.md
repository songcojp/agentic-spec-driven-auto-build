...
Use this skill when the operator, scheduler, or another skill requests `03.hld.define-state-flow` or when the current Agentic Spec phase requires this exact capability. Do not use it as a catch-all replacement for adjacent skills in the same phase.
