---
name: 06.planning.build-task-dag
description: "Run the Agentic Spec planning build task dag workflow. Use when the scheduler, operator, or another skill explicitly requests `06.planning.build-task-dag` and needs traceable planning decisions, task DAGs, dependency resolution, adapter selection, or startability findings inside the execution-planning boundary."
---

# Planning Build Task Dag

## Purpose

Run the exact Agentic Spec planning `build-task-dag` step for `06.planning.build-task-dag`. This skill turns referenced inputs into planning decisions, task DAGs, dependency resolution, adapter selection, or startability findings while staying inside the execution-planning boundary. It must preserve existing IDs, states, and evidence links unless the invocation explicitly authorizes a change.

## Codex Skill Usage

Use this project-local skill only when the user, scheduler, or another skill explicitly names `06.planning.build-task-dag` or the current SpecDrive workflow step requires it. Keep context lean: read referenced files from disk, pass paths/IDs/section anchors instead of pasted documents, and return the project-local Skill output contract rather than free-form prose. Provider YAML files under `agents/` are UI/provider prompt metadata only; subagent roles and fallback rules belong in `SKILL.md`.

## When to Use

Use this skill when the operator, scheduler, or another skill requests `06.planning.build-task-dag` or when the current Agentic Spec phase requires this exact capability. Do not use it as a catch-all replacement for adjacent skills in the same phase.
...
1. Confirm the invocation requested `06.planning.build-task-dag`, identify the target artifact or object, and verify the current state allows this build task dag step.
