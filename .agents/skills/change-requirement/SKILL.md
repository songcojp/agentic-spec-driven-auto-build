...
- Use `requirement-intake-skill` when the item needs a brand-new stable `REQ-*`, `NFR-*`, or `EDGE-*` ID.
- Use `spec-evolution-skill` when an existing ID must be changed, corrected, deprecated, superseded, clarified, or re-traced.
