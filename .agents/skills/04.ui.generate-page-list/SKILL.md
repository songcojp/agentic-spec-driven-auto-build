---
name: 04.ui.generate-page-list
description: "Run the Agentic Spec UI generate page list workflow. Use when the scheduler, operator, or another skill explicitly requests `04.ui.generate-page-list` and needs traceable UI states, interactions, page lists, or UI specification artifacts inside the UI specification and interaction boundary."
---

# Ui Generate Page List

## Purpose

Run the exact Agentic Spec UI `generate-page-list` step for `04.ui.generate-page-list`. This skill turns referenced inputs into UI states, interactions, page lists, or UI specification artifacts while staying inside the UI specification and interaction boundary. It must preserve existing IDs, states, and evidence links unless the invocation explicitly authorizes a change.

## Codex Skill Usage

Use this project-local skill only when the user, scheduler, or another skill explicitly names `04.ui.generate-page-list` or the current SpecDrive workflow step requires it. Keep context lean: read referenced files from disk, pass paths/IDs/section anchors instead of pasted documents, and return the project-local Skill output contract rather than free-form prose. Provider YAML files under `agents/` are UI/provider prompt metadata only; subagent roles and fallback rules belong in `SKILL.md`.

## When to Use

Use this skill when the operator, scheduler, or another skill requests `04.ui.generate-page-list` or when the current Agentic Spec phase requires this exact capability. Do not use it as a catch-all replacement for adjacent skills in the same phase.
...
1. Confirm the invocation requested `04.ui.generate-page-list`, identify the target artifact or object, and verify the current state allows this generate page list step.
...
3. Produce the UI states, interactions, page lists, or UI specification artifacts requested by the invocation, preserving stable IDs, states, source refs, and evidence links unless the invocation explicitly allows a change.
...
6. If this invocation generates or updates a page-list or other Spec document,
   run the mandatory quality review and repair loop from
