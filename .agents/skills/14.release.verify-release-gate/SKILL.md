---
name: 14.release.verify-release-gate
description: "Run the Agentic Spec release verify release gate workflow. Use when the scheduler, operator, or another skill explicitly requests `14.release.verify-release-gate` and needs traceable release-gate, PR-prep, release note, archive, or released-state outputs inside the release and delivery boundary."
---

# Release Verify Release Gate

## Purpose

Run the exact Agentic Spec release `verify-release-gate` step for `14.release.verify-release-gate`. This skill turns referenced inputs into release-gate, PR-prep, release note, archive, or released-state outputs while staying inside the release and delivery boundary. It must preserve existing IDs, states, and evidence links unless the invocation explicitly authorizes a change.

## Codex Skill Usage

Use this project-local skill only when the user, scheduler, or another skill explicitly names `14.release.verify-release-gate` or the current SpecDrive workflow step requires it. Keep context lean: read referenced files from disk, pass paths/IDs/section anchors instead of pasted documents, and return the project-local Skill output contract rather than free-form prose. Provider YAML files under `agents/` are UI/provider prompt metadata only; subagent roles and fallback rules belong in `SKILL.md`.

## When to Use

Use this skill when the operator, scheduler, or another skill requests `14.release.verify-release-gate` or when the current Agentic Spec phase requires this exact capability. Do not use it as a catch-all replacement for adjacent skills in the same phase.
...
1. Confirm the invocation requested `14.release.verify-release-gate`, identify the target artifact or object, and verify the current state allows this verify release gate step.
