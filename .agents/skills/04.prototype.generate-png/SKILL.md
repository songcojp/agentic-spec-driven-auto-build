---
name: 04.prototype.generate-png
description: "Run the Agentic Spec prototype generate png workflow. Use when the scheduler, operator, or another skill explicitly requests `04.prototype.generate-png` and needs traceable prototype artifacts or prototype-to-spec validation findings inside the prototype-artifact boundary."
---

# Prototype Generate Png

## Purpose

Run the exact Agentic Spec prototype `generate-png` step for `04.prototype.generate-png`. This skill turns referenced inputs into prototype artifacts or prototype-to-spec validation findings while staying inside the prototype-artifact boundary. It must preserve existing IDs, states, and evidence links unless the invocation explicitly authorizes a change.

## Codex Skill Usage

Use this project-local skill only when the user, scheduler, or another skill explicitly names `04.prototype.generate-png` or the current SpecDrive workflow step requires it. Keep context lean: read referenced files from disk, pass paths/IDs/section anchors instead of pasted documents, and return the project-local Skill output contract rather than free-form prose. Provider YAML files under `agents/` are UI/provider prompt metadata only; subagent roles and fallback rules belong in `SKILL.md`.

## When to Use

Use this skill when the operator, scheduler, or another skill requests `04.prototype.generate-png` or when the current Agentic Spec phase requires this exact capability. Do not use it as a catch-all replacement for adjacent skills in the same phase.
...
1. Confirm the invocation requested `04.prototype.generate-png`, identify the target artifact or object, and verify the current state allows this generate png step.
...
6. If this invocation generates or updates prototype spec metadata or other Spec
