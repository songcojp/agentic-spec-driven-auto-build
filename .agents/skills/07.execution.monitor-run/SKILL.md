...
description: "Run the Agentic Spec execution monitor run workflow. Use when the scheduler, operator, or another skill explicitly requests `07.execution.monitor-run` and needs traceable execution references, events, run monitoring results, state updates, or implementation evidence inside the execution-state boundary."
---
...
1. Confirm the invocation requested `07.execution.monitor-run`, identify the target artifact or object, and verify the current state allows this monitor run step.
