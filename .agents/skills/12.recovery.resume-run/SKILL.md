---
name: 12.recovery.resume-run
description: "Run the Agentic Spec recovery resume run workflow. Use when the scheduler, operator, or another skill explicitly requests `12.recovery.resume-run` and needs traceable recovery classification, checkpoint, restore, resume, blocked-state, or validation outputs inside the recovery boundary."
---
...
1. Confirm the invocation requested `12.recovery.resume-run`, identify the target artifact or object, and verify the current state allows this resume run step.
