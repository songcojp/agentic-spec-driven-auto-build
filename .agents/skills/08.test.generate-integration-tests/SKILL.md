...
description: "Execute the Agentic Spec 08 test workflow for generate integration tests with reusable input references, output contract, and acceptance checks."
---
...
Use this skill to perform the Agentic Spec `08` `test` workflow step for `generate-integration-tests`. Keep the workflow reusable across Agentic Spec projects and avoid product-specific assumptions.

## When to Use

Use this skill when the operator, scheduler, or another skill requests `08.test.generate-integration-tests` or when the current Agentic Spec phase requires this exact capability. Do not use it as a catch-all replacement for adjacent skills in the same phase.
