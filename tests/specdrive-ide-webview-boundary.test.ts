...
test("VSCode Feature Spec Webview switches between list and dependency graph views", () => {
  ...
  assert.match(extensionSource, /reviewReason === "clarification_needed"/);
  assert.match(extensionSource, /return clarifyFeatureButton\(feature\)/);
  assert.match(extensionSource, /reviewReason === "risk_review_needed"/);
  assert.match(extensionSource, /reviewFeatureButton\("Review", feature, "Feature Detail"\)/);
  assert.match(extensionSource, /reviewReason === "approval_needed"/);
  assert.match(extensionSource, /reviewFeatureButton\("Approve", feature, "Feature Detail"\)/);
  assert.match(extensionSource, /disabledButtonHtml\("Review", "Review reason is missing; refresh the Feature or open Execution Workbench\."/);
  assert.match(extensionSource, /disabledButtonHtml\("Approval", "Resolve the pending approval request in Execution Workbench\."/);
