...
test("VSCode Execution Workbench requires selected queue tasks for stateful actions", () => {
  ...
  assert.match(extensionSource, /reviewItemButton\(selectedItem\)/);
...
test("VSCode Feature Spec Webview switches between list and dependency graph views", () => {
  ...
  assert.match(extensionSource, /return `\$\{clarifyFeatureButton\(feature\)\}\$\{markFeatureReadyButton\("Ready", feature, projectId, "Feature Detail"\)\}`/);
