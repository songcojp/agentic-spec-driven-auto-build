...
- **弱いトレーサビリティ**：コード変更を PRD、ユーザーストーリー、受け入れ基準、設計境界に対応付けることが困難です。
...
10. 日常の実行、Feature Spec、品質エビデンス、レビューは VSCode IDE Webview で扱い、Product Console は互換性、システム設定、キューのデバッグ、全体状態確認のために維持する。

SpecDrive は Git、CI/CD、Issue Tracker、完全なプロジェクト管理システムを置き換えるものではありません。それらの上に、Spec-first な AI 自律開発オーケストレーション層を提供します。

---

## 現在の IDE Workbench スクリーンショット
...
| --- | --- |
| ![Execution Workbench](docs/agentic-spec/ui-v2/execution-workbench-runner.png) | ![システム設定](docs/agentic-spec/ui-v2/system-settings.png) |

| 監査とレビュー |
| --- |
| ![監査とレビュー](docs/agentic-spec/ui-v2/audit-review.png) |
...
ユーザー / PM / 開発者
        |
        v
VSCode IDE Webview / Product Console 互換サーフェス
        |
        v
Control Plane API
        |
...
| 概念 | 説明 |
| --- | --- |
| **Mainline Spec** | プロダクトレベルの事実源。PRD、ユーザーストーリー、HLD、UI Spec、Prototype Spec、変更ルールを含みます。 |
...
入力には、自然言語要求、既存 PRD、プロダクト brief、Pull Request、Issue、業務文書、既存コードベースなどを使用できます。システムはまず、それらを Mainline Spec に変換します。現在のリポジトリでは、MVP の主要な計画ソースは中国語の Mainline 文書であり、英語と日本語の文書はプロダクト向けナビゲーションとして維持されています。
...
├── apps/
│   ├── product-console/       # 設定とデバッグ用の React / Vite 互換 Console
│   └── vscode-extension/      # 主要な VSCode 拡張と IDE Webview Workbench
├── docs/agentic-spec/
│   ├── en/                    # 英語のプロダクト / Spec 文書
...
- [Product Requirements Document](docs/agentic-spec/ja/PRD.md)
...
SpecDrive AutoBuild は現在、活発に実装中です。このリポジトリには、Control Plane Runtime、Scheduler、永続化と監査の基盤、CLI / RPC Execution Adapter、Product Console 互換サーフェス、VSCode IDE Webview、Feature Specs、主要ワークフローのテストが含まれています。

現在のプロダクト方向では、VSCode IDE Webview が実行、Feature Spec、品質エビデンス、ReviewItem、システム設定の主要な日常 UI です。Product Console は、歴史的互換性、システム設定、adapter 設定、キューのデバッグ、全体状態確認のために維持されます。
...
1. [docs/agentic-spec/ja/PRD.md](docs/agentic-spec/ja/PRD.md)
...
1. `docs/agentic-spec/features/` 配下の関連 Feature Spec を読む。
