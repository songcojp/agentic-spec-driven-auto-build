...
2. 项目级 `schedule_run` 或 `start_auto_run` 读取 `docs/features/feature-pool-queue.json`，调用 `plan-feature-execution`，不再保留独立 `push_feature_spec_pool` 步骤，也不进行平台二次 `feature.select` / `feature.plan`。
