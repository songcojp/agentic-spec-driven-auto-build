...
| Feature Selection Skill Bridge | 将 Feature Pool Queue、`spec-state.json`、依赖、最近 Execution Record 和 resume/skip hints 交给 `06.planning.replan`，接收 `select_next_feature` 决策。 |
...
2. 项目级 `schedule_run` 或 `start_auto_run` 读取 `docs/features/feature-pool-queue.json`，调用 `06.planning.replan`，不再保留独立 `push_feature_spec_pool` 步骤，也不进行平台二次 `feature.select` / `feature.plan`。
