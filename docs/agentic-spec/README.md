# Documentation

Default language: English

Languages: English | [中文](zh-CN/README.md) | [日本語](ja/README.md)

---

## Documents

* [Product Requirements Document](en/PRD.md)
* [English User Stories](en/requirements.md)
* [Chinese Product Requirements Document](zh-CN/PRD.md)
* [Chinese User Stories](zh-CN/requirements.md)
* [Project HLD](zh-CN/hld.md)
* [Feature Spec Index](features/README.md)
* [Agentic Spec Standard](zh-CN/agentic-spec-standard.md)
* [Project Skill Guide](zh-CN/skills.md)

## Directory Layout

```text
docs/agentic-spec/
  en/       English documentation
  zh-CN/    Simplified Chinese documentation and current MVP planning source
  ja/       Japanese documentation
  features/ Feature Spec pool, state, dependencies, and delivery notes
  ui-v2/    Current IDE Workbench screenshots
```
