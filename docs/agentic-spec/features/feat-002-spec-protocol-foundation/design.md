...
|---|---|
| Spec Source Scanner | 扫描 PRD、用户故事、requirements、HLD、design、Feature Spec、tasks 和 README / 索引等来源，输出路径、类型、追踪关系、缺失项、冲突项和澄清项。 |
| Requirement Intake | 解析自然语言、PR、RP、PRD、用户故事 或混合格式输入。 |
| User Story Generator | 生成原子化、可测试、带来源追踪的 User Story。 |
...
1. 阶段 2 开始时，Spec Source Scanner 自动扫描当前项目的 PRD、用户故事、requirements、HLD、design、Feature Spec、tasks 和 README / 索引。
2. Requirement Intake 合并扫描结果和用户上传或提交的需求输入。
3. User Story Generator 生成 Requirement 和验收候选。
