...
对于 Feature 级 `feature_execution`，`implement-feature` 必须把 Feature Spec 目录作为实现范围：先读取 `requirements.md`、`design.md`、`tasks.md`，再由 agent 自主执行 Feature 内部任务并修改代码、测试、配置或必要文档。Scheduler / Execution Adapter 不追踪 Feature 内 task 状态；仅创建报告 JSON、仅复述计划、或把 `tasks.md` 标记为完成而没有实际产物，都不得视为成功输出。
