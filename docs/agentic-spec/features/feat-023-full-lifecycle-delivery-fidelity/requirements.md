...
- [ ] `.agents/skills/use-specdrive-lifecycle` 作为元技能存在，并能把任务路由到 Define、Plan、Build、Verify、Review、Ship 生命周期。
- [ ] `implement-feature`、`verify-behavior`、`review-delivery-evidence`、`review-delivery-evidence`、`review-delivery-evidence` 明确消费或产出 Delivery Fidelity 证据。
...
- [ ] 新增 `review-delivery-evidence`，跨 PRD -> requirements -> HLD -> UI Spec -> Feature Spec 审计颗粒度，失败时输出 `review_needed` 和 `intent_gap`、`behavior_gap`、`architecture_gap`、`interaction_gap`、`state_data_gap`、`task_gap`、`evidence_gap`。
