...
- CHG-034（2026-05-03）：用户确认 VSCode Feature Spec Webview 中 Feature 身份必须从 `docs/agentic-spec/features/README.md` 获取，数据库 Feature 记录和非 index 目录不得生成 Feature 列表项；目录扫描只用于校验 index 中的 folder 和读取三件套。影响 REQ-084 和 FEAT-021，作为完成 Feature 的 follow-up 修订。
...
- CHG-051（2026-05-07）：用户曾要求 VSCode Feature Spec Webview 自动刷新默认开启；打开 Feature Spec Webview 后立即显示开启状态并启动 extension host 自动刷新定时器。影响 REQ-084 和 FEAT-021；该默认开启行为已由 CHG-067 替代为默认关闭。
...
- CHG-067（2026-05-17）：用户反馈 IDE 操作界面自动刷新没有默认关闭；Execution Workbench、Spec Workspace 和 Feature Spec Webview 首次打开时必须默认关闭自动刷新，不启动 extension host 定时器，用户显式打开 switch 后才按 60 秒间隔刷新。影响 REQ-084 和 FEAT-021，替代 CHG-051 的默认开启行为。

## UI 概念图

| Webview | 概念图 |
|---|---|
| Execution Workbench | `docs/agentic-spec/ui/feat-021-execution-workbench-concept.png` |
| Spec Workspace | `docs/agentic-spec/ui/feat-021-spec-workspace-concept.png` |
| Feature Spec | `docs/agentic-spec/ui/feat-021-feature-spec-concept.png` |
...
- [x] `Spec Workspace` 展示 PRD、User Stories、HLD、UI Spec、Architecture Plan、Data Model、Contracts、Tasks、Quickstart、Execution、Review、Delivery 的全流程状态，并为当前阶段提供受控推进操作。
...
- [x] New Requirement、Requirement Change 和 Clarification 的 Skill 调用必须以 `feature_spec_ready_for_execution` 为目标：处理完成时同步 Feature Spec 三件套、`docs/agentic-spec/features/README.md`、`docs/agentic-spec/features/feature-pool-queue.json` 和 Feature `spec-state.json`，并将 Feature 置为 `ready` 以便用户直接从 UI 调度执行；无法 ready 时必须返回 blocked / review_needed。
- [x] `Feature Spec` 刷新时必须以 `docs/agentic-spec/features/README.md` 作为 Feature 身份来源；数据库 Feature 记录和未写入 index 的目录不得生成 Feature 列表项。刷新仍读取 index 中 folder 对应的 `requirements.md` / `design.md` / `tasks.md`，并识别缺失 folder、缺失三件套和状态冲突。
- [x] 因需求新增流程未经过 Feature 拆分而导致 `docs/agentic-spec/features/README.md` 未更新时，Feature Spec Webview 不显示该目录为 Feature 列表项，也不显示独立 `Feature Index Sync` 信息区块；应由需求新增 Skill 或后续规格同步补齐 Feature index。
- [x] 需求新增 Skill 创建或更新 Feature Spec 时必须同步 `docs/agentic-spec/features/README.md`，写入 Feature ID、Feature、Folder、Status、Primary Requirements、Suggested Milestone 和 Dependencies。
...
- [x] VSCode IDE Webview 自动刷新默认关闭；Execution Workbench、Spec Workspace 和 Feature Spec 首次打开时不得启动自动刷新定时器，用户显式打开 switch 后才由 VSCode extension host 按 60 秒间隔刷新，Webview 只渲染 switch 状态并提交 toggle 消息。
