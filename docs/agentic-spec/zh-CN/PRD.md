...
2026-05-03 自主执行选择更新：Project Scheduler 不再仅用代码中的 priority / dependency 过滤作为最终选择结论。启动自动执行或项目级调度运行时，系统应调用 `plan-feature-execution` 对 `feature-pool-queue.json`、Feature `spec-state.json`、依赖完成情况、最近 Execution Record、resume/skip hints 和 approval/block 状态进行推理，返回 `select_next_feature` 决策；代码只负责结构校验、安全闸和 `<executor>.run` Job 创建。CLI/app-server 返回的 `approval_needed`、`blocked`、`review_needed`、`failed` 必须投影到 Feature 执行结果，暂停自动继续。

2026-05-10 Git 生命周期边界更新：Feature 实现和交付采用 Skill-first 流程。`implement-feature` 负责创建或回退 Feature worktree、分支、提交、PR、检查、合并、远程分支清理、本地分支清理和 worktree 清理；平台代码只负责调度 `feature_execution`、传递 owner workspace 与 Feature Spec 路径、记录 Execution Record / run report / UI 投影，并校验 `SkillOutputContractV1.result.gitDelivery`。项目级并发时默认一个 Feature 一个 PR；Feature 内 worker worktree 只服务于同一个 Feature PR 的内部并发。
...
2026-05-12 质量优化 UI 边界更新：VSCode IDE Webview 是新增质量闭环的主操作界面。Execution Workbench 和 Feature Spec 详情必须优先展示 requirement coverage、acceptance evidence、journey evidence、runtime evidence、Delivery Fidelity、Git delivery、Workpad、日志、截图/trace、PR/check 和 ReviewItem 状态；Product Console 定位为历史遗留兼容面，仅保留系统设置、adapter 配置、队列调试和全局状态总览，不承载新的主质量 UI。
...
系统通过 `generate-user-stories` 生成用户故事文档，并通过独立的 `decompose-feature-specs` 将已确认需求拆分为 Feature Spec，同时产出机器可读的 `feature-pool-queue.json` 队列规划；拆分完成后不再存在独立“推入 Feature Spec Pool”步骤，任务调度全流程直接读取已生成的 Feature Spec 和队列规划结果来选择下一 Feature 并创建执行队列。Feature Spec 必须包含：
...
#### FR-011 PR/User Story 需求拆解

系统支持将用户提供的 PR、RP、PRD 片段或用户故事格式需求拆解为详细用户故事，并在需求质量检查通过后供 Feature Spec 拆分使用。
...
在阶段 2 需求录入开始时，系统必须自动扫描当前项目的 Spec Sources，包括 PRD、用户故事、requirements、HLD、design、已有 Feature Spec、tasks 和 README / 索引等文档，识别已有需求、设计、规划产物和缺口，并将扫描结果作为用户故事生成、澄清和质量检查的事实输入。阶段 2 允许扫描 HLD 和 Feature Spec 作为事实源，但不得在该阶段生成 HLD、拆分 Feature Spec 或启动规划流水线；这些操作属于阶段 3 的选中 Feature 受控操作。
...
Context: <state, surface, data, precondition>
Trigger: <user action, event, schedule, or external input>
Main Scenario: <normal path in observable steps>
Alternate / Negative Scenarios: <invalid, empty, permission, conflict, timeout, cancellation, or recovery paths>
Done Signal: <observable completion signal>
Source: <PRD/source/CQ refs>
...
US-012
As an App Creator,
I want to validate provider bindings before publishing,
so that I do not publish an app that cannot launch.

Context: The creator is editing an unpublished app draft with one provider binding.
Trigger: The creator clicks Validate Binding.
Main Scenario: The system runs validation, shows the result near the binding, and keeps Publish enabled only when validation passes.
Alternate / Negative Scenarios: Provider is missing, credentials are invalid, validation times out, or the creator cancels validation.
Done Signal: The creator sees the exact blocking reason or a passed validation state.
Source: PRD §6.2; CQ-004
Priority: Must
```

用户故事内容生成必须遵守 `docs/agentic-spec/requirements/user-stories-standard.md`。该标准约束用户故事和行为需求的生成颗粒度，不要求固定 Markdown 结构，也不得被 Feature Spec、设计文档、任务计划或实现 checklist 替代。

拆解结果必须包含：

* feature candidate
* detailed user story: actor、context、goal、reason/value、trigger、main scenario、alternate/negative scenarios、done signal
* requirement id
* condition/event
* expected behavior
* acceptance criteria
...
每条 用户故事必须保留源文本追踪关系，并能映射到 User Story、Acceptance Criteria、Evidence、Feature Spec 和后续测试用例。`US-*` 不得停留在“管理设置”“配置供应商”“发布应用”等功能区描述，必须细化到一个用户目标和可观察完成信号；`REQ-*`、`NFR-*` 和 `EDGE-*` 行必须从详细故事派生，并保持原子、可观察和可测试。无法从来源确定的 actor、trigger、状态、阈值、权限、数据、UI、运行时或证据决策必须写入 `CQ-*`，不得由生成 Skill 自行编造。
...
design-architecture
  ↓
design-architecture + design-architecture（可并行）
  ↓
decompose-feature-specs → 生成任务图
  ↓
Feature 状态 → tasked
```
...
* 任意项目级并行写入必须由 `implement-feature` 为每个 Feature 创建独立 Git worktree 和隔离分支
...
Project Scheduler 负责读取 `feature-pool-queue.json` 队列规划，调用 `plan-feature-execution` 推理选择下一项可执行 Feature，校验技能决策后创建 `<executor>.run` Job、记录 Execution Record 和跨 Feature 资源约束。Feature 内任务排序、并行、worker worktree 和 task 状态由 `implement-feature` 与 Feature Spec `tasks.md` 管理，平台不再维护二次 TaskGraph，也不直接创建 Feature worktree、分支或 PR。
...
* Stage 2 需求录入操作生成 `collect-project-context`、`generate-user-stories`、`manage-spec-change` 或 `validate-requirements` 的调用提示，并通过 CLI Adapter 在项目 workspace 中执行。
* Stage 3 planning 操作通过 CLI Skill Planning Bridge 调用 Skill：项目级 HLD 生成使用 `design-architecture` 并输出 `docs/agentic-spec/zh-CN/hld.md`；UI Spec 生成使用 `design-ui-spec`；per-Feature 规划继续调用 `implement-feature`、`plan-feature-execution`、`design-architecture`、`design-architecture`、`design-architecture`、`plan-feature-execution`、`decompose-feature-specs` 和 `review-code-spec`。
* Task Board 的运行操作通过 `implement-feature` 执行已排期任务；该 Skill 拥有 Feature worktree、分支、提交、PR、merge 和 cleanup 生命周期，平台代码只验证 `result.gitDelivery` 并投影状态。
...
系统通过 `use-specdrive-lifecycle` 元技能和项目 Skill 文档为任务选择 workflow、skill 与 agent persona。Product Interpreter、Requirement Critic、Interaction Designer、Task Slicer、Implementation Agent、Test Engineer、Browser QA、Code Reviewer 和 Release Reviewer 是责任角色；当运行时无法创建真实 subagent 时，owner thread 也必须按角色执行独立 pass 并记录 fallback。

#### FR-123 Skill Contract V2

`implement-feature` 返回 completed 的 `feature_execution` 必须使用 `skill-contract/v2`。v2 在现有 requirementCoverage、acceptanceEvidence、journeyEvidence 和 gitDelivery 之外，强制包含 `result.deliveryFidelity`。当变更触碰 UI/App 行为时，v2 还必须提供 `result.runtimeEvidence`，覆盖 app launch、route、用户操作、状态变化、reload 持久化或等价断言、负向/边界路径和 screenshot / trace / log 证据；foundation 或 stateless 变更必须用 `result.runtimeExemption` 显式豁免。`skill-contract/v1` 仍可读取 legacy 或非 Feature 输出，但不得作为新 Feature completed 的依据。
...
Agentic Spec 必须采用 requirements-first 的规格生成顺序：先确认 PRD 与 user stories 是否足够具体，再生成 HLD / UI Spec / Feature Spec design，最后生成 tasks。需求变更后必须 refine design 并 sync tasks，不得让粗颗粒度主线文档或 Feature Spec 直接进入实现。
...
#### FR-126 VSCode Webview Quality Workbench
...
循环由调用该 loop 的生成 Skill 选择本次 Quality Review Skill / Repair Owner，并由主生成线程定义完整 `qualityLoopPlan` 后分别委派 Quality Review Subagent 与 Repair Subagent 执行。共享 loop 不维护产物类型到质检技能的中央路由表；主生成线程不得自己把失败项静默修掉，只负责合并结构化结果、判断是否继续和选择退出状态。每次循环必须先确定允许修复的文件、来源事实、禁止文件、允许 gap 类型、风险上限、ID 策略、是否允许下游同步、所选质检技能及选择理由、修复 owner 及选择理由，并为每个 review / repair dispatch 提供可见的 subagent 名称和任务描述，例如 `Quality Review: <artifact>`、`Spec Repair: <artifact>`。当运行时支持 subagent/Task 工具时，生成 Skill 必须显式调用该工具，并把可见名称放入 dispatch description 或 prompt 首行，使 Codex 聊天记录能显示实际激活的 subagent；只有在现有来源足够、修复位于范围内且不会新增产品/架构决策时，才允许 Repair Subagent 修改产物。

每次文档生成最多执行 10 轮质量检测/修复。质量通过即返回完成；没有可修复项、剩余项不在修复范围、需要用户澄清、需要风险评审、重复同一 gap 指纹或达到 10 轮时，必须退出到 `clarification_needed`、`review_needed`、`risk_review_needed` 或 `blocked`，不得继续推进到 HLD、UI Spec、Feature 拆分、任务生成、ready、planning 或 execution。`result.qualityRepairLoop.subagents[]` 必须记录每个 subagent 或 owner-thread fallback pass 的 `displayName`、`dispatchDescription`、`activationEvidence`、角色、轮次、状态和证据引用，使执行 UI 与交付证据都能显示具体 subagent 名称；仅写入计划或 JSON 记录不得视为 subagent 已激活。
...
* 阶段 2 需求录入：将 Spec 来源扫描和上传合并为一个阶段内步骤，并在该步骤中显示“扫描”和“上传”两个动作；继续展示识别 PR/RP/PRD/用户故事、生成用户故事文档、完成关键澄清、执行需求质量检查的状态、阻塞原因和事实来源。
...
Product Console 必须支持 VS Code、浅色、深色和高对比度主题；首次打开且没有已保存偏好时默认使用浅色主题，用户切换主题后必须保留偏好并在刷新后恢复。
...
面板支持以下意图类型：查询任务状态、查询 Review 待审批项、新增需求、变更需求、触发调度（schedule_run）、暂停/恢复 Runner、批准/拒绝 Review、生成用户故事、生成 HLD、帮助说明、取消和确认。
...
SpecDrive IDE 是 VSCode 原生交互层。它承担日常 Spec 阅读、澄清、需求新增/变更、任务队列管理、执行观察、质量证据审查和 app-server 审批；Product Console 保留为系统设置、adapter 配置、队列调试和全局状态总览的历史兼容面。VSCode 插件必须提供独立 Execution Workbench Webview，作为任务调度、自动执行和质量闭环主界面，不复用当前 Product Console Web UI。

VSCode 插件能力包括：

* 识别 `docs/agentic-spec/<language>/PRD.md`、`requirements.md`、`hld.md`、`docs/agentic-spec/features/README.md`、`feature-pool-queue.json` 和各 Feature `spec-state.json`。
* 在 Activity Bar / Explorer 中展示 Spec Explorer，包括 PRD、User Stories、HLD、Feature Specs、Task Queue、Execution Record 和最近 Codex 会话。
* 在插件 Webview 中展示独立 Execution Workbench，默认聚焦 Job 队列、当前运行、下一步动作、阻塞原因、自动执行控制、审批待办和执行结果观察。
* 在 Execution Workbench 和 Feature Spec 详情中展示质量证据，包括 requirement coverage、acceptance evidence、journey evidence、runtime evidence、Delivery Fidelity、Git delivery、Workpad、日志、截图/trace、PR/check 和 ReviewItem 状态。
...
* 将用户在行级或段落级提交的澄清、需求新增、需求变更、用户故事生成、设计更新和 Feature 拆分意图转换为 `SpecChangeRequestV1`。
...
* Product Console 不承载新增主质量 UI；除非历史接口或兼容测试依赖，不得把新增质量展示优先实现到 Product Console。
...
* PR/User Story Decomposition Skill
* Clarification Skill
* Checklist Skill
* Constitution Skill
...
* `use-specdrive-lifecycle` 元技能和 agent persona registry。
...
* Runtime Evidence Gate、Invocation Context Manifest、Run Workpad 和 VSCode Webview Quality Evidence 展示。
