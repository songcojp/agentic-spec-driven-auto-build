...
- [ ] 平台调度器、Adapter 和 UI 代码不得直接创建 Feature 实现 worktree、提交、PR 或 merge；这些 Git 生命周期动作由 `implement-feature` 或补交付 Skill 执行。
...
验收：
- [ ] PR 内容可以追踪到需求、任务和证据。
- [ ] `implement-feature` 默认完成 PR 创建、PR checks 检查、允许时合并、远程分支清理、本地分支清理和 worktree 清理；策略要求分离交付时才转交 `prepare-release`。
