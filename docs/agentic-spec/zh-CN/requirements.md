...
2026-05-01 调度队列重构：平台只维护执行队列 Job 和 Execution Record / 执行记录。Job 类型表示 executor（如 `cli.run`、`native.run`），payload `operation` 表示任务操作（如 `feature_execution`、`generate_user_stories`、`generate_hld`、`generate_ui_spec`、`split_feature_specs`）。Feature/Task/Project 不再是 Job 顶层属性，只能出现在 payload `context` 中。`feature.select`、`feature.plan`、`feature_planning`、FeatureSelectionDecision、平台 TaskGraph 写入和 `feature.plan` blocked 语义均废弃；Feature 内部 task 状态由 LLM 与 Feature Spec `tasks.md` 管理。

## 2. 目标

- 将自然语言、PR、RP、PRD 或用户故事输入转化为结构化 Feature Spec。
...
### REQ-005：拆解 PRD 为 用户故事
来源：PRD 第 6.2 节 FR-011；`docs/agentic-spec/requirements/user-stories-standard.md`
优先级：Must

WHEN Spec Protocol Engine 处理 PR、RP、PRD、用户故事 或混合格式需求
THE SYSTEM SHALL 将行为拆解为原子化、可测试、带来源追踪的 用户故事。

验收：
- [ ] 每条需求只描述一个可观察行为，并映射到 Feature Spec、验收标准和测试场景。
- [ ] `US-*` 必须包含 actor、context、goal、reason/value、trigger、main scenario、alternate/negative scenarios、done signal、source refs 和 priority；不得停留在功能区、模块名或一句话愿望。
- [ ] `REQ-*`、`NFR-*` 和 `EDGE-*` 行必须从详细 `US-*` 派生，包含稳定 ID、来源、`US-*` 映射或 system invariant 理由、用户故事/可测语句、验收、证据类型、优先级和状态。
- [ ] `EDGE-*` 必须覆盖详细故事中的错误、边界、权限、空态、冲突、无效输入和负向路径，除非来源明确将其标为不做范围。
- [ ] 缺少 actor、trigger、状态、阈值、权限、数据、UI、运行时或证据决策时必须生成 `CQ-*`，相关 Must 需求保持 `review_needed`。
...
- [ ] 平台调度器、Adapter 和 UI 代码不得直接创建 Feature 实现 worktree、提交、PR、merge 或 cleanup；这些 Git 生命周期动作由 `implement-feature` 编排 `setup-worktree`、`clean-worktree` 或补交付 Skill 执行。
...
THE SYSTEM SHALL 从 `feature-pool-queue.json` 和 Feature `spec-state.json` 动态读取 ready Feature，并调用 `plan-feature-execution` 选择下一个 Feature。
...
WHEN 项目级调度触发
THE SYSTEM SHALL 读取 `feature-pool-queue.json` 中已经规划好的 Feature 队列，调用 `plan-feature-execution` 执行 `select_next_feature` 推理选择，并把通过代码安全校验的工作转换为 `<executor>.run` Job。

验收：
- [ ] Project Scheduler 不依赖 Project Memory 中的静态候选队列作为真实调度来源。
- [ ] `plan-feature-execution` 的 `result` 至少返回 `decision`、`featureId`、`reason`、`blockedReasons`、`dependencyFindings`、`resumeRequiredFeatures` 和 `skippedFeatures`。
...
- [ ] Spec/Feature 流程状态以 `docs/agentic-spec/features/<feature-id>/spec-state.json` 为机器可读事实源，数据库只保存运行时执行事实；Feature 状态使用 `status`，最近执行状态使用 `executionStatus`，队列状态动作必须同步两者与 Execution Record。
...
- [ ] `setup-worktree` 产出 owner workspace、implementation workspace、worktree、branch、base commit、target branch、Worktree Mode 和 setup 证据；`clean-worktree` 产出 commit、PR、checks、merge、远程分支清理、本地分支清理和 worktree cleanup 证据。
...
- [ ] `codex-cli` preset 支持 Codex CLI Fast mode 但默认不启用：adapter defaults 使用 `serviceTier=standard`、`fastMode=false`，默认命令不传 `service_tier` 覆盖，仅传递 `features.fast_mode=false`；当 `serviceTier=fast` 时传递 `service_tier="fast"`，旧配置中的 `serviceTier=flex` 兼容归一化为 `standard`。
...
- [ ] `skillInstruction` 至少包含 `skillName`、`requestedAction`、`sourcePaths`、`expectedArtifacts`、`imagePaths` 和可选 `operatorInput`；系统不再生成独立 `SkillInvocationContractV1`。
...
- [ ] Stage 2 需求录入操作映射到需求扫描、需求拆解、需求新增或质量检查相关 Skill；Stage 3 planning 操作映射到 planning Skill pipeline；Feature 运行操作映射到 `implement-feature`。
...
验收：
- [ ] 恢复任务包含失败类型、失败命令、摘要、相关文件、历史尝试、禁止重试项和最大重试次数，且不包含固定 Skill name。
...
验收：
- [ ] PR 内容可以追踪到需求、任务和证据。
- [ ] `implement-feature` 默认通过 `clean-worktree` 完成 PR 创建、PR checks 检查、允许时合并、远程分支清理、本地分支清理和 worktree 清理；策略要求分离交付时才转交 `prepare-release`。
...
- [ ] Spec Protocol 初始化不得在目标项目创建 `docs/agentic-spec/change-management.md`、`docs/agentic-spec/zh-CN/change-management.md` 或 `docs/agentic-spec/*/change-disposition-checklist.md`；需求新增/变更协议必须由 `.agents/skills/manage-spec-change/SKILL.md` 承载。
...
来源：用户指令：阶段 2 自动扫描 PRD、用户故事、HLD、Feature Spec 等；用户指令：阶段 2 将 Spec 扫描和上传合成一个步骤并显示“扫描”“上传”两个按钮；PRD 第 5 节阶段 2；PRD 第 6.2 节 FR-011
优先级：Must

WHEN 项目完成阶段 1 初始化并进入需求录入
THE SYSTEM SHALL 自动扫描当前项目中的 PRD、用户故事、requirements、HLD、design、Feature Spec、tasks 和 README / 索引等 Spec Sources，识别已有需求格式、规格产物、来源追踪、缺失项和冲突，并将扫描结果提供给用户故事生成、澄清和需求质量检查。

验收：
- [ ] 阶段 2 扫描结果包含已发现的 PRD、用户故事、requirements、HLD、design、Feature Spec、tasks 和 README / 索引路径及其类型。
...
THE SYSTEM SHALL 调用 Codex CLI 对消息进行意图分类，识别意图类型（query_status、query_review、add_requirement、change_requirement、schedule_run、pause_execution_adapter、resume_execution_adapter、approve_review、reject_review、generate_user_stories、generate_hld、help、cancel、confirm、unknown），并在 Codex 不可用时退回基于关键词的规则分类。
...
WHEN Chat Interface 识别到低风险（query_status、query_review、help、cancel、confirm）或中等风险（generate_user_stories、generate_hld、add_requirement、change_requirement）意图
...
来源：`docs/agentic-spec/zh-CN/vscode-codex-rpc-prd.md` REQ-VSC-001；`docs/agentic-spec/zh-CN/vscode-app-plan.md` FEAT-016
优先级：Should
...
验收：
- [ ] 支持多语言结构：`docs/agentic-spec/<language>/PRD.md`、`docs/agentic-spec/<language>/requirements.md`、`docs/agentic-spec/<language>/hld.md`。
...
来源：`docs/agentic-spec/zh-CN/vscode-codex-rpc-prd.md` REQ-VSC-002、REQ-VSC-003；`docs/agentic-spec/zh-CN/vscode-app-plan.md` FEAT-016
优先级：Should
...
验收：
- [ ] 左侧树显示 PRD、User Stories、HLD、Feature Specs 和 Task Queue。
...
来源：`docs/agentic-spec/zh-CN/vscode-codex-rpc-prd.md` REQ-VSC-004 至 REQ-VSC-006；`docs/agentic-spec/zh-CN/vscode-app-plan.md` FEAT-017
优先级：Should

WHEN 用户打开 PRD、requirements、HLD 或 Feature Spec
THE SYSTEM SHALL 通过 VSCode Hover、CodeLens 和 Comments 提供行级/段落级 Spec 交互，允许用户提交澄清、需求新增、需求变更、生成用户故事、更新设计和拆分 Feature 的意图。
...
来源：`docs/agentic-spec/zh-CN/vscode-codex-rpc-prd.md` 第 7.4 节；`docs/agentic-spec/zh-CN/vscode-app-plan.md` FEAT-017
优先级：Must
...
来源：`docs/agentic-spec/zh-CN/vscode-codex-rpc-prd.md` 第 7.3 节；`docs/agentic-spec/zh-CN/vscode-app-plan.md` Interfaces And Contracts
优先级：Must
...
来源：`docs/agentic-spec/zh-CN/vscode-codex-rpc-prd.md` REQ-VSC-003、第 7.6 节；`docs/agentic-spec/zh-CN/vscode-app-plan.md` FEAT-019
优先级：Should
...
来源：`docs/agentic-spec/zh-CN/vscode-codex-rpc-prd.md` 第 7.7 节；`docs/agentic-spec/zh-CN/vscode-app-plan.md` FEAT-018
优先级：Must
...
来源：`docs/agentic-spec/zh-CN/vscode-codex-rpc-prd.md` 第 7.7 至 7.9 节；`docs/agentic-spec/zh-CN/vscode-app-plan.md` Interfaces And Contracts
优先级：Must
...
- [ ] `result` 必须是机器可读对象，允许不同 Skill 写入专用执行详情；调用端不得按 `skillName` 硬编码专用字段校验。
- [ ] 不新增重型 Execution Result；聊天记录、provider 事件流、raw logs 和 Execution Record 共同构成执行证据。

### REQ-082：支持 VSCode app-server 审批交互
来源：`docs/agentic-spec/zh-CN/vscode-codex-rpc-prd.md` 第 7.8 节；`docs/agentic-spec/zh-CN/vscode-app-plan.md` FEAT-019
优先级：Must
...
来源：`docs/agentic-spec/zh-CN/vscode-codex-rpc-prd.md` REQ-VSC-007；`docs/agentic-spec/zh-CN/vscode-app-plan.md` FEAT-020
优先级：Should
...
来源：`docs/agentic-spec/zh-CN/vscode-codex-rpc-prd.md` REQ-VSC-017；用户指令“为 VS 插件开发独立的 Web UI，不要复用现在的 Web UI，核心关注任务调度和自动执行”；用户指令“vscode ide ui 添加多语言支持，支持中文，英语，日语切换”
优先级：Must
...
THE SYSTEM SHALL 在需求新增、需求变更或澄清处理完成时同步推进到可直接实现的 Feature Spec：更新主线需求、HLD、`docs/agentic-spec/features/README.md`、受影响 Feature Spec 三件套、`docs/agentic-spec/features/feature-pool-queue.json` 和 Feature `spec-state.json`；完成后 Feature 必须处于可由 UI 直接调度执行的 `ready` 状态，除非存在明确阻塞原因并返回 blocked / review_needed。

WHEN VSCode Feature Spec Webview 刷新 Feature 列表
THE SYSTEM SHALL 同步读取 Feature index 和 `docs/agentic-spec/features/*` Feature 文件夹，并在发现 index 漏项时更新 Feature index 或返回明确的同步阻塞原因。
...
WHEN 用户首次打开 VSCode IDE Webview
THE SYSTEM SHALL 默认关闭自动刷新；Execution Workbench、Spec Workspace 和 Feature Spec 不启动自动刷新定时器，直到用户显式打开自动刷新 switch。
...
- [ ] Execution Workbench、Spec Workspace 和 Feature Spec Webview 首次打开时自动刷新 switch 默认关闭，extension host 不启动自动刷新定时器；用户显式打开 switch 后才按 60 秒间隔刷新，关闭 switch 后停止定时器。
...
- [ ] 顶部 New Feature 以弹出输入框收集自然语言需求，提交后不得由前端硬编码判断新增或变更；必须交给模型按 `manage-spec-change` / `manage-spec-change` 边界判定。
...
- [ ] 刷新 Feature Spec 视图时同时扫描 `docs/agentic-spec/features/README.md` 和 `docs/agentic-spec/features/*` 三件套目录；因需求新增不经过拆分流程导致 index 漏项时，刷新流程必须补齐 Feature index 或报告需要人工处理的冲突。
- [ ] 需求新增 Skill 在创建或更新 Feature Spec 后必须同步 `docs/agentic-spec/features/README.md`，写入 Feature ID、名称、Folder、Status、Primary Requirements、Suggested Milestone 和 Dependencies。
...
- [ ] `use-specdrive-lifecycle` 能把请求路由到生命周期、项目 Skill 和 agent persona。
...
WHEN `implement-feature` 返回 completed 的 `feature_execution`
...
- [ ] Requirements Gate 必须以 `docs/agentic-spec/requirements/user-stories-standard.md` 为 requirements 内容生成契约，确认 Must `US-*` 已细化到 actor、context、goal、reason/value、trigger、main scenario、alternate/negative scenarios 和 done signal，并确认每个 `REQ-*` / `NFR-*` / `EDGE-*` 是从详细故事派生的原子、可观察、可测试行为单元；不可直接转成测试的需求返回 `story_gap`、`atomicity_gap`、`behavior_gap`、`traceability_gap`、`clarification_gap` 或 `evidence_gap`。
...
- [ ] 每个文档生成 Skill 必须为本次 Quality Review Subagent 和 Repair Subagent 定义可见名称与 dispatch 描述，例如 `Quality Review: <artifact>`、`Spec Repair: <artifact>`；运行时支持 subagent/Task 工具时必须显式调用该工具，并把可见名称放入 dispatch description 或 prompt 首行，使 Codex 聊天记录能展示实际激活的 subagent。
- [ ] Skill-local quality loop 不得维护产物类型到质检技能的中央路由表；调用它的生成 Skill 必须根据本次产物、下游阶段和允许范围选择 Quality Review Skill 与 Repair Owner。
- [ ] 质量检测必须由调用方选择的 Quality Review Subagent 或等价隔离 review context 执行；仅写入计划、JSON 字段或 owner-thread 自审不得视为 subagent 已激活；结果必须按 gap ID 标记 `in_scope_repairable`、`in_scope_not_repairable` 或 `out_of_scope`。
...
- [ ] 生成 Skill 的 `result.qualityRepairLoop` 必须记录是否执行循环、最大轮次、已用轮次、最终决策、`qualityLoopPlan`、`subagents[]`（包含 `role`、`displayName`、`skillName`、`dispatchDescription`、`activationEvidence`、`round`、`status`、证据引用和 fallback 原因）、剩余缺口和退出原因。
...
WHEN MVP 运行在目标范围内
THE SYSTEM SHALL 追踪 Feature Spec 自动生成成功率、PR/User Story 拆解准确率、澄清问题有效率、任务图可执行率、低风险任务自动完成率、状态判断准确率、失败恢复率、PR 交付报告生成率和任务可追踪覆盖率。
