...
- [ ] `codex-cli` preset 支持 Codex CLI Fast mode 但默认不启用：adapter defaults 使用 `serviceTier=standard`、`fastMode=false`，命令模板传递 `service_tier="standard"` 和 `features.fast_mode=false`。
