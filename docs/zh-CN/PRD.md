...
* Stage 3 planning 操作通过 CLI Skill Planning Bridge 调用 Skill：项目级 HLD 生成使用 `design-architecture` 并输出 `docs/zh-CN/hld.md`；UI Spec 生成使用 `design-ui-spec`；per-Feature 规划继续调用 `implement-feature`、`plan-feature-execution`、`design-architecture`、`design-architecture`、`design-architecture`、`plan-feature-execution`、`decompose-feature-specs` 和 `review-code-spec`。
